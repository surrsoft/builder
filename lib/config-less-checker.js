      ...
      switch (typeof config[optionName]) {
         case 'string':
            if (!isGlob(config[optionName])) {
               throw new Error('паттерн для опции old в конфигурации по темизации задан ' +
         ...
         case 'object':
            if (config[optionName].include) {
               ...
               config[optionName].include.forEach((pattern) => {
                  if (!isGlob(pattern)) {
                     throw new Error('один из паттернов для опции old в конфигурации по темизации задан ' +
