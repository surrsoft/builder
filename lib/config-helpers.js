...
function clearSourcesSymlinksIfNeeded(cacheDirectory) {
   if (fs.pathExistsSync(path.join(cacheDirectory, 'cache-path.json'))) {
      const { lastCacheDirectory } = fs.readJsonSync(path.join(cacheDirectory, 'cache-path.json'));
      if (lastCacheDirectory !== cacheDirectory) {
