...
function parseJsComponent(text) {
      ...
      enter: function(node) {
         ...
         if (node.type === 'CallExpression' && node.callee.type === 'Identifier' &&
            ...
            if (node['arguments'][0].type === 'Literal' && typeof node['arguments'][0].value === 'string') {
               result.moduleName = node['arguments'][0].value;
            }
   ...
   if (arrExpr.length && returnStatement) {
      ...
      if (opts.hasOwnProperty('webPage')) {
         ...
         result.webPage = {};
         if (opts['webPage'].hasOwnProperty('htmlTemplate')) {
            result.webPage.htmlTemplate = opts['webPage']['htmlTemplate'];
         }
         if (opts['webPage'].hasOwnProperty('title')) {
            result.webPage.title = opts['webPage']['title'];
         }
         if (opts['webPage'].hasOwnProperty('outFileName')) {
