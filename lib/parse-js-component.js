...
function parseJsComponent(text, testsBuild) {
      ...
      enter(node) {
         ...
         if (isExpressionWithCall(node)) {
            ...
            if (searchResult && !expressionCallback.has('catch')) {
               needToReplace = true;
               const resultedAst = Object.assign({}, defaultCatchAst);
               resultedAst.callee.object = node.expression;
               modulesToReplace.push({
                  code: escodegen.generate(resultedAst),
                  start: node.range[0],
                  end: node.range[1]
               });
            }
   ...
   if (needToReplace) {
      result.patchedText = helpers.replaceStringsByIndex(text, modulesToReplace);
   }
   return result;
}
