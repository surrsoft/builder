...
var path = require('path'),
    jsShaper = require('jsshaper'),
    shaper = jsShaper.shaper,
    ref = jsShaper.ref;
...
(function () {
   ...
   /**
    * Ищет с помощью jsShaper функцию define, и берет ее параметры
    * @param {String} script
    * @param {Function} callback
    * @returns {Object}
    */
   function inDefine(script, callback){
      var root;
      try {
         root = shaper.parseScript(script);
      } catch (error) {
         throw error;
      }

      var define_template1 = shaper.parse('define($,$,$)'),
          define_template2 = shaper.parse('define($,$)'),
          define_visited = false;
      ...
      root = shaper.traverse(root, {
         pre: function (node) {
            ...
            var dependency_list, module_function, module_name;
            if (shaper.match(define_template1, node)) {
               dependency_list = node.children[1].children[1];
               module_function = node.children[1].children[2];
               module_name     = node.children[1].children[0];
            }
            else if (shaper.match(define_template2, node)) {
               dependency_list = node.children[1].children[0];
               module_function = node.children[1].children[1];
               module_name     = node.children[1].children[0];
            }
            else
               return;
            ...
            callback(module_name, dependency_list, module_function);
         }
      });
   ...
   function getDictionaryPath(grunt, obj, lang, path) {
      ...
      if (dirName) {
         ...
         try {
            ...
            text = grunt.file.read(modPath);
            if (text) {
               shaperCode = inDefine(text, function(modName, dependencyList) {
                  modName = modName.value.replace('js!', '');
                  ...
                  var new_dependency = 'i18n!' + modName;
                  for (var i = 0; i < dependencyList.children.length && !findDependency; i++) {
                     if (dependencyList.children[i].value.indexOf(new_dependency) >= 0) {
                        findDependency = true;
                     }
                  }

                  if (!findDependency) {
                     shaper.insertBefore(new ref(dependencyList, 'children', -1), shaper.parse('\'' + new_dependency + '\''), ', ', false);
                  }
               });

               if (!findDependency && shaperCode !== null) {
                  grunt.file.write(modPath, shaperCode.getSrc());
               }
            }
         } catch(e) {
   ...
   module.exports = {
       ...
       * @param {String} dictMask - Маска для словарей
       * @param {String} application
       */
      indexDict: function(grunt, languages, dictMask, application) {
