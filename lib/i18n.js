...
(function () {
   ...
   function isLanguageDir(subDir) {
      var isLangDir = subDir ? subDir.match(/^(..-..)\//) : false;
      if (isLangDir && (isLangDir[1] in dictionary)) {
         return true;
      } else if(subDir && subDir in dictionary) {
         return true;
      }
   ...
   function createLangStatic(language, application) {
      ...
      var availableLang = {},
          jsDict = {},
          local = language.substr(0,2).toLowerCase(),
          country = language.substr(3,2).toUpperCase(),
          lang = local + '-' + country,
   ...
   module.exports = {
       ...
       * @param application
       */
      translateAll: function(g, application) {
         if (!g) {
            return;
         } else {
            grunt = g;
       ...
       * @param application
       */
      translateOne: function(g, lang, application) {
         if (!g) {
            return;
         } else {
            grunt = g;
       ...
       * @param application
       */
      translateDef: function(g, application) {
         if (!g) {
            return;
         } else {
            grunt = g;
         ...
         recursive(application, function(absPath, rootDir, subDir, fileName) {
            if (/\.xhtml$/.test(fileName)) {
               copy(absPath, absPath, null, true);
            }
         });

         // На статике по умолчанию нет поддердживаемых языков
         replaceContents('availableLanguage', {}, application);
       ...
       * @param application
       */
      findDictionary: function(g, dictMask, application) {
         if (!g) {
            return;
         } else {
            grunt = g;
