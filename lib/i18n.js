...
(function () {
   ...
   function translate(text, lang) {
      return text.replace(/\{\[([\s\S]+?)\]\}/g, function(m, code) {
         return (lang && dictionary[lang] && dictionary[lang][code]) ? dictionary[lang][code] : code;
      });
   }
   ...
   function getDictionaryPath(obj, lang, path) {
      ...
      var langDir = 'resources/lang/' + lang + '/' + lang + '.json',
          ...
          text, modName, cssPath, cssCountryPath;
   ...
   function createLangStatic(language, application) {
      ...
      var jsDict = {},
          ...
          langDir = path.join(lang, application);
      ...
      recursive(application, function(absPath, rootDir, subDir, fileName) {
         ...
         copy(absPath, path.join(langDir, subDir ? subDir : '', fileName), lang, /\.xhtml$/.test(fileName));
      });
   ...
   function translateDef(defLang, application) {
      var jsDict = {},
          lang = defLang.substr(0,2).toLowerCase() + '-' + defLang.substr(3,2).toUpperCase();
      ...
      recursive(application, function(absPath, rootDir, subDir, fileName) {
         ...
         // Берем все xhtml файлы, Игнорируем папку ws в корне
         if (/\.xhtml$/.test(fileName) && !((new RegExp('^ws/|^' + application + '/ws/', '')).test(absPath))) {
