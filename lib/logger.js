...
class GruntLogger {
   ...
   error(message) {
      if (typeof message === 'string') {
         this._grunt.log.error(this._grunt.template.today('hh:MM:ss'), message);
      } else {
         this._grunt.log.error(this._grunt.template.today('hh:MM:ss'), this._prepareObjectMessage(message));
      }
   }

   warning(message) {
      if (typeof message === 'string') {
         this._grunt.log.warn(this._grunt.template.today('hh:MM:ss'), message);
      } else {
         this._grunt.log.warn(this._grunt.template.today('hh:MM:ss'), this._prepareObjectMessage(message));
      }
   }

   info(message) {
      this._grunt.log.ok(this._grunt.template.today('hh:MM:ss'), this._grunt.template.today('hh:MM:ss'), message);
   }

   debug(message) {
      this._grunt.log.debug(this._grunt.template.today('hh:MM:ss'), `${message}`);
   }
   ...
   _prepareObjectMessage(obj) {
      ...
      return GruntLogger._prepareStringMessage(this._grunt.template.today('hh:MM:ss'), `${messages.join(': ')}`);
   }
   ...
   */
   _prepareObjectMessage(type, obj) {
      ...
      if (obj.hasOwnProperty('code')) {
         codeStr = GulpLogger._normalizeCodeError(obj['code'].toString());
      }
      ...
      return GulpLogger._prepareStringMessage(type, `Builder_${codeStr} location: [${locations.join(', ')}] ${messages.join(': ')}`);
   }
