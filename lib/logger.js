...
class GulpLogger extends Logger {
   ...
   setBaseInfo(cloudName, cloudResponsible) {
      this._cloudInfo = {
         cloudName,
         cloudResponsible
      };
   ...
   error(message) {
      ...
      this._addMessageForReport('error', message, this._cloudInfo);
   }

   warning(message) {
      this._hasWarnings = true;
      this._consoleLog('WARNING', message);
      this._addMessageForReport('warning', message, this._cloudInfo);
   }
   ...
   */
   ...
   _addMessageForReport(level, obj, cloudInfo) {
      const result = {
         level,
         message: '',
         'cld_name': cloudInfo.cloudName,
         'cld_responsible': cloudInfo.cloudResponsible
      };
