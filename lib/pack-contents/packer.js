   ...
   function over(err) {
      if (err) {
         ...
         try {
            ...
            // FIXME: избавиться от перемещенных файлов
            // clean up
            //self._cleanup();
...
Packer.prototype._followTheSecondPrinciple = function (type, filename, moduleName, cb) {
         ...
         if (node.type === 'CallExpression' && node.callee && node.callee.name === 'define') {
            if (node.arguments && node.arguments[1] && node.arguments[1].type === 'ArrayExpression') {
               ...
               dependencies.forEach(function (d, i) {
                  if (d.value && common.belongs(d.value, moduleName, self._contents)) {
                     packet.push(d.value);
                  }
               });
            }
   ...
   packet = packet.map(function (d, i) {
      var
         ...
         relativePath = path.dirname(name.replace(moduleName, ''));

      // cut plugins such as browser?, is? and etc
      prefix = prefix.indexOf('?') !== -1 ? prefix.substring(prefix.indexOf('?') + 1) : prefix;
