...
function belongs(filename, moduleName) {
   return filename.indexOf(moduleName) !== -1 || (fromResources.test(filename) && !isModule.test(filename));
}
...
Packer.prototype._sort = function (cb) {
   ...
   function over(err) {
      if (err) {
         ...
         try {
            grunt.file.write(path.join(this._serviceRoot, this._resources, 'contents.js'), 'var contents = ' + JSON.stringify(self._contents));
            grunt.file.write(path.join(this._serviceRoot, this._resources, 'contents.json'), JSON.stringify(self._contents));
            cb(self._contents);
         } catch(err) {
...
Packer.prototype._followTheSecondPrinciple = function (type, filename, moduleName, cb) {
   ...
   grunt.file.recurse(path.dirname(file), function(abspath, rootdir, subdir, filename) {
      // firstly, check if it is module's dependency
      if(belongs(path.join(subdir, filename), moduleName)) {
         files.push({
            abspath: abspath,
            subdir: subdir,
            name: filename
         });
      } else if (isModule.test(filename)) {
         // yep, it is module, but not the same!
         // FIXME!
      }
   });
