...
Packer.prototype._followTheSecondPrinciple = function (type, filename, moduleName, cb) {
   ...
   var
      prefix = 'js!',
      links = self._dependencies.links[prefix.concat(moduleName)],
      nodes = self._dependencies.nodes;
   ...
   links && links.forEach(function(link) {
      // FIXME: странный критерий! Лучше вычитать исходную директорию?
...
Packer.prototype._shift = function (moduleName, files, destination, cb) {
   files.forEach(function(file) {
      var
         newFile = path.join(destination, moduleName + addExtname(file)),
         deleteOptions = {
            force: true
         };
      ...
      try {
         ...
         grunt.file.delete(file, deleteOptions);
         cb();
      } catch(err) {
         cb(err);
      }
