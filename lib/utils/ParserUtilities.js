...
Node.prototype.outerHTML = function () {
   ...
   return startTag + restoreWsExpertComments(this.innerHTML(), this._store) + closeTag;
};
...
function parse(markup) {
   var
      store = [],
      tags = markup instanceof Array ? markup : removeWsExpertComments(markup, store).replace(htmlComments, '').match(tagRegExp),
      result = new Node({
         nodeType: 9,
         childNodes: [],
         parentNode: null,
