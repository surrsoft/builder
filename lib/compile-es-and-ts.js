...
function normalizeDependencies(sourceCode, moduleName, interfaceModule) {
   const ast = acorn.parse(sourceCode, { range: true });
                     ...
                     for (let i = 0; i < argument.elements.length; i++) {
                        ...
                        if (newDependency !== dependency) {
                           ...
                           modulesToReplace.push({
                              ...
                              start: argument.elements[i].start,
                              end: argument.elements[i].end
                           });
                        }
                     }
                     break;
...
function compileEsAndTs(relativePath, text, interfaceModule) {
   ...
   return normalizeDependencies(result.outputText, moduleName, interfaceModule);
}
