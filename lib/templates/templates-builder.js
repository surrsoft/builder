   ...
   render(_options) {
      return new Promise((resolve, reject) => {
         global.requirejs(['UI/Base'], (base) => {
            ...
            const { appRoot, wsRoot, resourceRoot, RUMEnabled, pageName, servicesPath } = this;
            try {
               resolve(base.BaseRoute({
                  application, appRoot, wsRoot, resourceRoot,
                  RUMEnabled, pageName, servicesPath, _options
               }));
            }
            catch (e) { reject(e); }
         }, reject);
