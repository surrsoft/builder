...
class TemplatesBuilder {
   ...
   async render(_options) {
      const html = await new Promise((resolve, reject) => {
         global.requirejs(['UI/Base', 'SbisEnv/PresentationService'], (UIBase, PresentationService) => {
            const config = {
               servicesPath: this.servicesPath,
               resourceRoot: this.resourceRoot,
               application: 'UI/_base/HTML',
            ...
            const route = UIBase.BaseRoute;
            try {
               // #region
               // ! FIXME после доброски https://online.sbis.ru/opendoc.html?guid=59f87611-f87f-404e-bb27-06350248d6b2
               this.appInit.default(config, PresentationService.default, new UIBase.StateReceiver());

               // this.appInit.startRequest(config, new UIBase.StateReceiver());
               // #endregion
