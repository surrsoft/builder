...
async function buildTmpl(text, relativeFilePath, componentsProperties, templateExt) {
   ...
   if (templateExt === 'wml') {
      ...
      if (Object.keys(componentsProperties).length > 0) {
         buildConfig.createResultDictionary = true;
         buildConfig.componentsProperties = componentsProperties;
      }
      return promiseTimeout.promiseWithTimeout(new Promise((resolve) => {
         templateRender.getFile(text, buildConfig, function callbackFn(funcText) {
            resolve({
