...
/**
 * check template for optional script existance.
 * Ignore it because of s3mod can contain only
 * strict dependencies.
 * @param{Object} file - current template file object
 * @param{String} versionModuleName - current module name
 * @returns {boolean}
 */
function checkForOptionalScriptsIfNeeded(file, versionModuleName) {
   if (file.history[0].endsWith('.xhtml') || file.history[0].endsWith('.html')) {
      const moduleExistCondition = new RegExp(`\\.hasModule\\(['"]${versionModuleName}['"]\\)`);
      return moduleExistCondition.test(file.contents);
   }
   return false;
}
...
function versionizeStyles(file, moduleInfo) {
   ...
   const newText = content.replace(
      ...
      (match, partUrl, partFilePath, partExt, extraData) => {
         ...
         /**
          * WS.Core depends on many platform modules, that has dependence from WS.Core.
          * Ignore it to prevent cycle dependencies
          * Ignore "Intest" Interface module from external dependencies checker, patches
          * will not be used for control's integration tests
          */
      ...
      .replace(
         ...
         (match, partEqual, partFilePath, partExt) => {
            ...
            if (moduleInfo.name !== 'WS.Core' && moduleInfo.name !== 'Intest') {
               ...
               if (checkResult.error && !checkForOptionalScriptsIfNeeded(file, versionModuleName)) {
                  errors = true;
                  logger.error({
                     filePath: file.path,
                     moduleInfo,
             ...
             * we have to make links to "bundles" and "contents" unique.
             */
            if (/(}|\/)(bundles|contents)/.test(partFilePathWithoutMin)) {
