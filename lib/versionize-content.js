...
function versionizeStyles(file, moduleInfo, currentVersion) {
   ...
   const newText = content.replace(
      ...
      (match, partUrl, partFilePath, partExt, extraData) => {
         ...
         /**
          * In multi and single service application builds we are forced
          * to add x_version header to avoid server misunderstanding of which
          * static server should be used for content giving back.
          */
         if (extraData) {
            const remainingHeaders = extraData.slice(1, extraData.length);
            result += `?${currentVersionHeader}`;
            result += `&x_version=${currentVersion}${remainingHeaders.startsWith('#') ? '' : '#'}${remainingHeaders}`;
         } else {
            result += `?${currentVersionHeader}`;
