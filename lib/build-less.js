...
function resolveUrls(text, appRoot) {
   return text.replace(/url\(['"]?([^'"]+)['"]?\)/g, (str, filePath) => {
      let prettyFilePath = filePath;
      if (prettyFilePath.includes('..') && prettyFilePath.includes('/SBIS3.CONTROLS/')) {
         let newFolder = '/resources/SBIS3.CONTROLS';
         if (appRoot && appRoot !== '/') {
            newFolder = helpers.prettifyPath(path.join(appRoot, newFolder));
         }
         prettyFilePath = prettyFilePath.replace(/.*SBIS3\.CONTROLS/, newFolder);
      }
      return `url('${prettyFilePath}')`;
   });
}
   ...
   return new Promise(async function(resolve, reject) {
      try {
         ...
         try {
            ...
            resolve({
               text: resolveUrls(output.css, appRoot),
