 ...
 */
async function getCompiledLess(lessContent, filePath, pathsForImport, theme, imports) {
   ...
   const processor = postcss([
      autoprefixer({ grid: true, browsers: browsersList, remove: false })
   ]);
...
async function processLessFile(data, filePath, theme, gulpModulesInfo) {
   ...
   let lessResult;
   try {
      lessResult = await getCompiledLess(newData, filePath, pathsForImport, theme, imports);
   } catch (error) {
      if (error instanceof LessError) {
         ...
         if (error.type === 'File') {
            logger.info(`file ${filePath} failed to build because of import error. Trying to rebuild`);

            // sometimes less compiler doesn't see existing import files. Try to rebuild to solve problem
            try {
               lessResult = await getCompiledLess(newData, filePath, pathsForImport, theme, imports);
            } catch (fileError) {
               return {
                  error: `${errorLineStr}: ${fileError.message}`,
                  failedLess: fileError.filename,
                  theme
               };
            }
            logger.info(`rebuild of file ${filePath} finished successfully!`);
            return lessResult;
         }
...
async function buildLess(
   ...
   const newThemes = lessInfo.newThemes ? lessInfo.newThemes : styleThemes;
   ...
   if (!notThemedLess && !prettyRelativeFilePath.includes('themes/') &&
      ...
      await pMap(
         ...
         async(themeName) => {
            const currentTheme = {
               name: themeName,
               path: newThemes[themeName].path
            };
            if (newThemes[themeName].hasOwnProperty('config')) {
               currentTheme.config = newThemes[themeName].config;
            }
