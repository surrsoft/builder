   ...
   return new Promise(async function(resolve, reject) {
      try {
         ...
         if (sbis3ControlsPath) {
            ...
            let variablesPath = path.join(themesPath, theme, '_variables'),
               mixinsPath = path.join(themesPath, '_mixins');

            //костыль для поддержки обратной совместимости в тестах builder'а и sbis3.controls
            //удалить после 01.05.2018
            if (!await fs.pathExists(variablesPath + '.less')) {
               variablesPath = path.join(themesPath, theme, 'variables');
               mixinsPath = path.join(themesPath, 'mixins');
            }

            imports = [
               `@SBIS3CONTROLS: '${sbis3ControlsPath}';`, //TODO: убрать в 3.18.300 после 01.05.2018
...
function buildLess(filePath, text, modulePath, sbis3ControlsPath, pathsForImport, appRoot) {
   return new Promise(async function(resolve, reject) {
      try {
         ...
         try {
            resolve(await processLessFile(text, prettyFilePath, theme, prettySbis3ControlsPath, pathsForImport, appRoot));
         } catch (error) {
            reject(error);
         }
      } catch (error) {
