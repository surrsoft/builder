   ...
   if (!notThemedLess && !isThemePath && lessInfo.themes) {
      await pMap(
         ...
         async(themeName) => {
             ...
             * TODO спилить когда будет внедрено конфигурирование темизации на уровне Интерфейсного модуля
             */
            if (isWS4RetailPrestoLess && builderConstants.oldThemes.includes(themeName)) {
               return;
            }

            /**
             * розница и престо полностью несовместимы с темой default-Layout, не билдим её для
             * данных Интерфейсных модулей.
             * TODO спилить, когда будет внедрено конфигурирование темизации на уровне Интерфейсного модуля.
             */
            if ((prettyFilePath.includes('/Presto/') ||
               prettyFilePath.includes('/Retail/')) &&
               themeName === 'default-Layout') {
               return;
            }
