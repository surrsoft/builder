...
async function processLessFile(data, filePath, theme, sbis3ControlsPath, pathsForImport, appRoot) {
   ...
   /**
    * импортим миксины в зависимости от наличия SBIS3.CONTROLS в проекте
    */
   if (sbis3ControlsPath) {
         ...
         postcssUrl({
            url(asset) {
               ...
               if (asset.url.includes('..')) {
                  ...
                  if (asset.url.includes('/ui-modules/Controls-theme/')) {
                     let newFolder = '/resources/Controls-theme';
                     if (appRoot && appRoot !== '/') {
                        newFolder = helpers.prettifyPath(path.join(appRoot, newFolder));
                     }
                     return asset.url.replace(/.*\/ui-modules\/Controls-theme/, newFolder);
                  }
               }
               return asset.url;
            }
...
async function buildLess(filePath, text, modulePath, sbis3ControlsPath, pathsForImport, allThemes, appRoot) {
