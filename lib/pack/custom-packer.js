...
async function saveBundlesForEachModule(taskParameters, applicationRoot, result) {
   /**
    * write bundlesRoute meta by interface modules names.
    */
   const jsonToWrite = {};
   await pMap(
      ...
      async(currentModule) => {
         ...
         let intModuleName;
         if (moduleNameWithoutPlugins.includes('/')) {
         ...
         if (taskParameters.config.sources && !result.excludedCSS.hasOwnProperty(currentModule)) {
