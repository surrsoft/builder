 ...
 */
function splitCssByThemes(cssModules, themesList, compiledLess) {
   const result = {};
   cssModules.forEach((module) => {
      const
         moduleName = module.moduleYes ? module.moduleYes.fullName : module.fullName,
         cssName = moduleName.replace(/.*css!(theme\?)?/, '');

      const compiledLessForCurrentCss = compiledLess
         .filter(currentCss => currentCss.startsWith(cssName))
 ...
 */
async function saveCustomPackResults(taskParameters, result, applicationRoot) {
   const bundlesPath = 'WS.Core/ext/requirejs',
      bundlesRoutePath = path.join(bundlesPath, 'bundlesRoute').replace(/\\/g, '/'),
      pathsToSave = {
         bundles: path.join(applicationRoot, bundlesPath, 'bundles.js')
      };

   if (taskParameters.config.sources) {
      pathsToSave.bundlesJson = path.join(applicationRoot, bundlesPath, 'bundles.json');
   ...
   if (taskParameters.config.dependenciesGraph && !resourcesUrl) {
      await fs.writeJson(
         path.join(
            applicationRoot,
   ...
   if (taskParameters.config.modules instanceof Array) {
      await pMap(
         ...
         async(moduleInfo) => {
            ...
            taskParameters.compiledLessByModules[moduleName] = await fs.readJson(
               path.join(root, moduleName, '.builder/compiled-less.min.json')
            );
         },
         {
            concurrency: 20
