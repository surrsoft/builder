...
async function writeCustomPackage(packageConfig, applicationRoot) {
   ...
   if (packageConfig.cssModulesFromOrderQueue.length > 0) {
      result.unshift(packHelpers.generateLinkForCss(packageConfig.cssModulesFromOrderQueue, packageConfig.packagePath));
   }
...
function appendCurrentBundlesOptionsToCommon(current, common, idProperty) {
   ...
   await pMap(
      ...
      async(config) => {
         ...
         try {
            ...
            appendCurrentBundlesOptionsToCommon(currentResult, results, 'bundles');
            appendCurrentBundlesOptionsToCommon(currentResult, results, 'bundlesRoute');
