...
async function generateCustomPackage(
   ...
   /**
    * если существуют исключённые css, они могут быть записаны в jsный пакет, надо передавать его для
    * jinnee, чтобы не пропускать placeholder версионирования
    */
   if (Object.keys(excludedCSS).length > 0) {
      taskParameters.versionedModules[packagePath.split('/')[0]].push(`${packagePath}.js`);
   }
   ...
   if (cssModulesFromOrderQueue.css.length > 0) {
      ...
      await pMap(
         ...
         async(themeName) => {
            ...
            taskParameters.versionedModules[packagePath.split('/')[0]].push(`${packagePath}.css`);
         },
         {
            concurrency: 10
         }
