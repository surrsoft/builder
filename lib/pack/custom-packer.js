...
async function writeCustomPackage(packageConfig, applicationRoot, splittedCore) {
   ...
   if (packageConfig.cssModulesFromOrderQueue.length > 0) {
      result.unshift(packHelpers.generateLinkForCss(
...
async function generateCustomPackage(depsTree, applicationRoot, packageConfig, isSplittedCore, isGulp) {
...
async function saveBundlesForEachModule(applicationRoot, result, isGulp) {
   ...
   await pMap(
      ...
      async(currentBundle) => {
         const
            intModuleName = currentBundle.match(/^resources\/([^/]+)/)[1],
 ...
 */
async function generatePackageJsonConfigs(depsTree, configs, applicationRoot, isSplittedCore, isGulp) {
   const results = {
      bundles: {},
      bundlesRoute: {}
   };
