...
async function writeCustomPackage(packageConfig, applicationRoot, splittedCore, availableLanguage, defaultLanguage) {
...
async function generateCustomPackage(
   ...
   if (packageConfig.platformPackage || !packageConfig.includeCore) {
      const cssBundlePath = pathToCustomCSS.replace(applicationRoot.replace(/\\/g, '/'), '');
   ...
   await writeCustomPackage(packageConfig, applicationRoot, isSplittedCore, availableLanguage, defaultLanguage);
   return result;
}
