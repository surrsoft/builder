                  ...
                  .map(function onlyPath(module) {
                     ...
                     if (!taskParameters.config.sources && !cssIsPackageOutput) {
                        ...
                        removeFromVersioned.forEach((moduleToRemove) => {
                           logger.info(`Удаляем стиль ${module.fullPath}`);
                           const moduleIndex = currentVersionedModules.indexOf(moduleToRemove);
                           logger.info(`Индекс стиля в мета-файле ${moduleIndex}. Файл присутствует до удаления: ${currentVersionedModules.includes(moduleToRemove)}`);
                           currentVersionedModules.splice(moduleIndex, 1);
                           logger.info(`Файл присутствует в мета-файле после удаления: ${taskParameters.versionedModules[moduleName].includes(moduleToRemove)}`);
                        });
                     }
