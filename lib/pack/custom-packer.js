...
async function saveCustomPackResults(taskParameters, result, applicationRoot, splittedCore) {
   ...
   if (taskParameters.config.version) {
      await pMap(
         ...
         async(currentModule) => {
            const pathForVersionMeta = path.join(applicationRoot, currentModule, '.builder/versioned_modules.json');
            logger.info(`CustomPack. Записали мета-файл versioned_modules для модуля "${currentModule}" по пути ${pathForVersionMeta}`);
            await fs.writeJson(
               pathForVersionMeta,
