      ...
      await pMap(
         ...
         async(themeName) => {
            ...
            if (packageConfig.extendsTo) {
               const extendCssPath = pathToCustomCSS.replace(root, '');
...
async function joinAndSaveExtendableBundles(extendBundles, applicationRoot) {
   ...
   const wsCoreBundles = await fs.readJson(wsCoreBundlesPath);
   ...
   await fs.outputJson(path.join(wsCoreBundlesPath), wsCoreBundles);
}
...
async function generateCommonExtendBundles(jsonToWrite, applicationRoot, extendBundles) {
   ...
   await pMap(
      ...
      async(currentExtendBundle) => {
         ...
         result[
            extendBundles[currentExtendBundle].extendsTo
         ].content.add(currentExtendBundle);
         result[
            extendBundles[currentExtendBundle].extendsTo
         ].config[currentExtendBundle] = extendBundles[currentExtendBundle].config;
...
async function saveBundlesForEachModule(taskParameters, applicationRoot, result) {
   ...
   await joinAndSaveExtendableBundles(commonExtendBundles, applicationRoot);
}
