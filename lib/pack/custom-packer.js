...
async function _generateCustomPackage(depsTree, applicationRoot, packageConfig, isSplittedCore) {
   ...
   if (packageConfig.platformPackage || !packageConfig.includeCore) {
      result.bundles[packagePath] = await packHelpers.generateBundle(orderQueue, cssModulesFromOrderQueue, isSplittedCore);
   ...
   await pMap(
      ...
      async config => {
         ...
         try {
             ...
             */
            const currentResult = await _generateCustomPackage(depsTree, applicationRoot, config, bundlesOptions.splittedCore);
