 ...
 * @returns {Promise<void>}
 */
async function joinAndSaveExtendableBundles(taskParameters, extendBundles, applicationRoot, result) {
   const wsCorePaths = {
   ...
   await pMap(
      ...
      async(currentExtendBundle) => {
         ...
         taskParameters.addFileForRemoval(path.join(applicationRoot, 'WS.Core', bundleWithoutRoot));
      }
   );

   await fs.outputJson(wsCorePaths.wsCoreBundlesPath, wsCoreBundles);
   taskParameters.addFileForRemoval(wsCorePaths.wsCoreBundlesPath);
   await fs.outputJson(wsCorePaths.wsCoreBundlesRoutePath, wsCoreBundlesRoute);
   taskParameters.addFileForRemoval(wsCorePaths.wsCoreBundlesRoutePath);
}
...
async function saveBundlesForEachModule(taskParameters, applicationRoot, result, modulesForPatch) {
   ...
   if (needToSaveExtendBundlesMeta) {
      ...
      taskParameters.addFileForRemoval(path.join(applicationRoot, 'common-extend-bundles.json'));
   }
   ...
   if (needToSaveExtendBundlesMeta) {
      await joinAndSaveExtendableBundles(
         taskParameters,
         commonExtendBundles,
         helpers.prettifyPath(applicationRoot),
         result
      );
   }
}
      ...
      await pMap(
         ...
         async(currentModule) => {
            ...
            if (modulesForPatch.length === 0 || modulesForPatch.includes(currentModule)) {
               ...
               await fs.outputJson(outputVersionedMeta, taskParameters.versionedModules[currentModule].sort());
   ...
   await pMap(
      ...
      async(currentModule) => {
         ...
         if (modulesForPatch.length === 0 || modulesForPatch.includes(currentModule)) {
            ...
            await fs.outputJson(outputCdnMeta, taskParameters.cdnModules[currentModule].sort());
