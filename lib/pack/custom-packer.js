...
async function writeCustomPackage(packageConfig, applicationRoot, application, buildNumber) {
   ...
   if (packageConfig.cssModulesFromOrderQueue.length > 0) {
      result.unshift(
         packHelpers.generateLinkForCss(packageConfig.cssModulesFromOrderQueue, application, packageConfig.packagePath, buildNumber)
      );
   }

   if (packageConfig.optimized) {
      await fs.writeFile(packageConfig.outputFile, packHelpers.generatePackageToWrite(result));
   } else {
      await fs.writeFile(
...
async function generateCustomPackage(depsTree, applicationRoot, packageConfig, bundlesOptions) {
   const
     isSplittedCore = bundlesOptions.splittedCore, outputFile = packHelpers.getOutputFile(packageConfig, applicationRoot, depsTree, isSplittedCore),
   ...
   await writeCustomPackage(packageConfig, applicationRoot, bundlesOptions.application, bundlesOptions.buildNumber);
   return result;
}
...
async function saveBundlesForEachModule(applicationRoot, result) {
   ...
   await pMap(
      ...
      async(currentBundle) => {
         const bundlePath = path.normalize(
               path.join(applicationRoot, `${currentBundle.match(/^resources\/[^/]+/)}`, 'bundlesRoute.json')
            ),
...
async function getAllConfigs(files, applicationRoot) {
   ...
   await pMap(
      files,
      async(file) => {
         const currentFileConfigs = await packHelpers.getConfigsFromPackageJson(
            path.join(applicationRoot, file),
            applicationRoot
         );
   ...
   await pMap(
      ...
      async(config) => {
         ...
         try {
             ...
             */
            const currentResult = await generateCustomPackage(depsTree, applicationRoot, config, bundlesOptions);
