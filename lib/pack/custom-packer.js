...
async function generateCustomPackage(
    ...
    */
   if (!packageConfig.level || packageConfig.level !== 'Application') {
...
/**
 * Создаёт кастомный пакет по переданной конфигурации кастомной паковки.
 * @param {DependencyGraph} depsTree граф зависимостей
 * @param {Object} writtenModules - модули, записанные в супер-бандлах
 * @param {Object} currentConfig текущая конфигурация кастомной паковки
 * @param {String} root корень приложения
 * @param {String} application название сервиса
 * @param isSplittedCore
 * @param isGulp
 * @param availableLanguage доступные языки
 * @param defaultLanguage язык по умолчанию
 * @returns {Promise<{bundles: {}, bundlesRoute: {}}>}
 */
async function generateCurrentConfigPackage(
   depsTree,
   writtenModules,
   currentConfig,
   root,
   ...
   try {
      ...
      logger.warning({
         message: `Ошибка создания кастомного пакета по конфигурационному файлу ${currentConfig.packageName} - ${configNum} - ${currentConfig.output}`,
         error: err,
...
async function compileCurrentPackage(taskParameters, depsTree, writtenModules, currentConfig, results, root) {
   const currentResult = await generateCurrentConfigPackage(
      depsTree,
      writtenModules,
      currentConfig,
      root,
