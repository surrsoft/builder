...
async function rebaseCSS(css, appRoot, packagePath) {
...
async function customPackCSS(files, root, packagePath) {
   const results = await pMap(
      files,
      async(css) => {
         const result = await rebaseCSS(css, root, packagePath);
         return result;
      },
      {
         concurrency: 10
...
async function saveModuleCustomPackResults(taskParameters, result, applicationRoot, modulesForPatch) {
   ...
   await pMap(
      ...
      async(currentModule) => {
         ...
         // skip saving libraries meta into bundlesRoute if there is nothing to save
