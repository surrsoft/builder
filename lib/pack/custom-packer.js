...
async function generateCustomPackage(
   ...
   const applicationRoot = path.join(root, application),
      ...
      cssCurrentTheme = {},
      excludedCSS = {},
      isCommonPackage = !packageConfig.level || packageConfig.level !== 'Service';
   ...
   orderQueue = packHelpers.getOrderQueue(depsTree, packageConfig, excludedCSS, applicationRoot).filter((node) => {
       ...
       */
      if (isCommonPackage && writtenModules.hasOwnProperty(node.fullName)) {
         return false;
      }
 ...
 * @param {String} root корень приложения
 * @param {Object} intersects пересечения между кастомными пакетами
 * @returns {Promise<void>}
 */
async function splitIntersectsByUiModuleName(root, intersects) {
   const intersectsByUiModules = {};

   Object.entries(intersects).forEach((currentEntry) => {
      const
         currentModule = currentEntry[0],
         currentModuleIntersect = currentEntry[1],
   ...
   await pMap(
      ...
      async(currentEntry) => {
         const
            currentModuleName = currentEntry[0],
            currentModuleIntersects = currentEntry[1],
            intersectOutput = path.join(root, `${currentModuleName}/customPackIntersects.json`);
         logger.info(
            `В Интерфейсном модуле ${currentModuleName} присутствуют пересечения между кастомными пакетами!` +
            ` Посмотреть можно в json-файле по пути ${intersectOutput}`
         );
         await fs.outputJson(intersectOutput, currentModuleIntersects);
      },
      {
         concurrency: 10
      }
...
async function collectAllIntersects(root, results) {
   ...
   Object.entries(bundles).forEach((currentEntry) => {
      ...
      currentBundle.forEach((module) => {
         if (!commonModules.hasOwnProperty(module)) {
            commonModules[module] = [currentBundleName];
         } else if (intersects.hasOwnProperty(module)) {
            intersects[module].push(currentBundleName);
         } else {
            intersects[module] = commonModules[module];
            intersects[module].push(currentBundleName);
         }
      });
   });

   await splitIntersectsByUiModuleName(root, intersects);
}
