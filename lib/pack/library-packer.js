...
async function dependencyWalker(
   ...
   if (!currentDepTree.has(currentDependency)) {
      ...
      if (!checkDependencyForExisting(dependencyContent.moduleName, result)) {
         ...
         privatePartsForChangesStore.push(dependencyContent.moduleName);
      }
...
async function packCurrentLibrary(sourceRoot, privatePartsForChangesStore, data, privateModulesCache) {
   const
