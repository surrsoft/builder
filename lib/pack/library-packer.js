...
function removeStrictModeStatement(functionCallbackBody, topLevelReturnStatement, exportsDefine) {
   if (astring.generate(functionCallbackBody.body[0]).includes('\'use strict\';')) {
...
async function packLibrary(sourceRoot, data, privateModulesCache) {
         ...
         if (node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'define') {
            ...
            if (exportsDefine.position) {
               ...
               if (topLevelReturnStatement) {
                   ...
                   */
                  if (astring.generate(returnArgument) !== 'exports') {
                     logger.error({
               ...
               if (topLevelReturnStatement) {
                  const returnArgument = topLevelReturnStatement.statement.argument;
                  exportsObject = astring.generate(returnArgument);
            ...
            privateDependenciesOrder.forEach((dep) => {
               ...
               try {
                  functionCallbackBody.body.splice(
                     packIndex,
                     0,
                     acorn.parse(`exports['${dep.moduleName}'] = true;`)
                  );
                  packIndex++;
               } catch (error) {
                  const errorMessage = `Acorn error: cant parse Javascript code: exports['${dep.moduleName}'] = true;\n` +
               ...
               const
                  moduleCode = `var ${currentDependencyName} = function(){ ${isTemplate ? '' : '"use strict";'} 
                     var exports = {};\nvar result =` +
                     `(${astring.generate(dep.ast)})(${argumentsForClosure});\n` +
               ...
               try {
                  functionCallbackBody.body.splice(
                     packIndex,
                     0,
                     acorn.parse(moduleCode)
                  );
                  packIndex++;
               } catch (error) {
                  const errorMessage = `Acorn error: cant parse generated code for private library part: ${moduleCode}\n` +
                     `Error message: ${error.message} \n Stack: ${error.stack}`;
                  throw new Error(errorMessage);
               }
               let
                     ...
                     if (nativeJavascriptNames.includes(libraryParametersNames[privateDependencyIndex].name)) {
                        ...
                        try {
                           functionCallbackBody.body.splice(
                              packIndex,
                              0,
                              acorn.parse(aliasCode)
                           );
                           packIndex++;
                        } catch (error) {
                           const errorMessage = `Acorn error: cant parse generated code for alias: ${aliasCode}\n` +
                              `Error message: ${error.message} \n Stack: ${error.stack}`;
                           throw new Error(errorMessage);
                        }
                     }
            ...
            functionCallbackBody.body.push(acorn.parse(currentReturnExpressions.join('\n')));
   ...
   if (privatePartsForCache.length > 0) {
      ...
      libraryResult.packedModules = privatePartsForCache;
   }
