...
function dependencyWalker(
   ...
   if (!currentDepTree.has(currentDependency)) {
      const dependencyContent = libPackHelpers.readModuleAndGetParamsNames(
         root,
         libraryName,
         currentDependency,
         libraryDependenciesMeta,
...
function recursiveAnalizeEntry(
   ...
   /**
    * Не будет ругаться через throw, просто проставим флаг.
    * Это позволит нам полностью проанализировать граф и сразу
    * выдать разработчикам все проблемные места. залоггируем
    * все проблемы сразу.
    */
...
function packCurrentLibrary(root, privatePartsForChangesStore, data, privateModulesFiles) {
   ...
   let privateDependencies = Object.keys(libraryDependenciesMeta).filter(
      dependency => libraryDependenciesMeta[dependency].isLibraryPrivate
   );
   let privateDependenciesOrder = [];

   try {
      privateDependencies = recursiveAnalizeEntry(
         root,
         libraryName,
         libraryDependenciesMeta,
         externalDependenciesToPush,
   ...
   /**
    * собираем для всех приватных модулей, что импортирует к себе библиотека, метаданные
    * для дальнейшего использования - набор зависимостей, callback в формате ast и имя
    * самого приватного модуля.
    */
   privateDependencies.forEach(function(dependency) {
      let result;
      try {
      } catch (error) {
         logger.warning({
