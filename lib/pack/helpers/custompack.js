...
function getOrderQueue(dg, cfg, applicationRoot) {
   ...
   return commonPackage
      ...
      .filter((module) => {
         if (exclude) {
            return !exclude.test(module.fullName);
         }
         if (excludeCoreReg) {
...
function generateBundlesRouting(currentBundle, pathToBundle, cssBundleConfig) {
   ...
   for (let i = 0; i < currentBundle.length; i++) {
      ...
      const isBadEngine = helpers.unixifyPath(cssBundlePath).includes('SBIS3.ENGINE/Controls/engineclipboard');
       ...
       */
      if (!currentBundle[i].startsWith('css!SBIS3.CONTROLS')) {
         if (currentBundle[i].indexOf('css!') === 0) {
            // TODO спилить костыль про SBIS3.ENGINE, как будет решение по ошибке
            // https://online.sbis.ru/opendoc.html?guid=8cda1f13-7287-475f-994f-4e1c20d3a11b
            if (!isBadEngine) {
