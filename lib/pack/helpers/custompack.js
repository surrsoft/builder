   ...
   for (let i = 0; i < currentBundle.length; i++) {
      if (currentBundle[i].indexOf('css!') === 0) {
         if (!excludedCSS.hasOwnProperty(currentBundle[i])) {
            bundlesRoute[currentBundle[i]] = `${helpers.unixifyPath(rebasedBundlePath)}` +
...
function getCssPackagePathForMeta(themeName, packagePath) {
   let cssPackagePath;
   if (themeName) {
      if (packagePath.endsWith('.package.min')) {
         cssPackagePath = packagePath.replace(/\.package\.min$/, `_${themeName}.package.min.css`);
      } else {
         cssPackagePath = packagePath.replace(/\.min$/, `_${themeName}.min.css`);
      }
   } else {
      cssPackagePath = `${packagePath}.css`;
   }
   return cssPackagePath;
}
