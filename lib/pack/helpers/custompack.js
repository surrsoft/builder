 ...
 * @return {Array}
 */
async function getConfigsFromPackageJson(configPath, applicationRoot, cfgContent) {
   const
      configsArray = [],
      packageName = path.basename(configPath);

   /**
    * На гальпе мы уже будем передавать прочитанный конфиг, поскольку он
    * считывается через gulp.src
    */
   if (!cfgContent) {
...
function generateLinkForCss(cssModules, packagePath) {
   const linkHref = packagePath.replace(/\.min$/, '');
   let result = '(function(){';
