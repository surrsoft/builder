...
function sortModulesForDependencies(orderQueue) {
   orderQueue.forEach((currentModule) => {
      function watcher(dependency, currentDepth, maxDepth) {
         ...
         if (founded) {
            maxDepth += 1;
            founded.deps && founded.deps.forEach((dep) => {
      ...
      currentModule.deps && currentModule.deps.forEach((dep) => {
   ...
   orderQueue = orderQueue.sort((a, b) => {
      if (a.depth > b.depth) {
         return 1;
      } if (a.depth < b.depth) {
         return -1;
...
function collectExternalDependencies(orderQueue) {
   ...
   orderQueue.forEach((currentModule) => {
      ...
      currentModule.deps && currentModule.deps.filter((dep) => {
         ...
         return orderQueue.filter(module => module.moduleName === dep).length === 0;
      }).forEach((dep) => {
...
async function generatePackageToWrite(listOfModules) {
   ...
   if (listOfModules) {
      ...
      /* если для конкретного модуля помимо явного define существует define
           ...
           */
      orderQueue = orderQueue.filter(currentModule => !(!currentModule.ifStatementIncluded && orderQueue.find(module => currentModule.moduleName === module.moduleName && module.ifStatementIncluded)));
      ...
      orderQueue.forEach((moduleToWrite) => {
                  ...
                  if (node.expression.type == 'CallExpression' && node.expression.callee.type == 'Identifier' && node.expression.callee.name == 'define') {
                     let
                        ...
                        writeForm = `Object.defineProperty(exports, '${moduleToWrite.moduleName}', {\n
                                            ...
                                            return exports['${moduleToWrite.moduleName}'] = ${generatedCallBack}(${moduleToWrite.deps ? moduleToWrite.deps.map((dep) => {
                           let
                              moduleInPackage = orderQueue.find(module => module.moduleName === dep),
         ...
         const allDeps = [];
         moduleToWrite.deps && moduleToWrite.deps.forEach((currentDep) => {
            const dep = getAllDepsRecursively(currentDep, allDeps, orderQueue);
            if (!allDeps.includes(dep)) {
               allDeps.push(dep);
            }
         });
         defineOfModules += `\ndefine('${moduleToWrite.moduleName}', [${allDeps.map(dep => `'${dep}'`)}], function() {return exports['${moduleToWrite.moduleName}'];});`;
      });
