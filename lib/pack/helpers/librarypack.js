...
function isPrivate(dependency) {
   ...
   dependencyParts.forEach((part, index) => {
      if (part.startsWith('_')) {
         /**
          * Для зависимостей, имеющих префикс "__" именно в самом имени файла,
          * делаем исключение. Такие зависимости только в Controls(отдельные исключения будут
          * обговариваться), будут исправлены в рамках перевода Controls на библиотеки.
          */
         if (!result && part.startsWith('__') && index === dependencyParts.length - 1) {
            return;
         }
         result = true;
      }
...
async function readModuleAndGetParamsNames(
   ...
   let moduleData = getCacheByModuleName(privateModulesCache, normalizedCurrentDependency);
    ...
    */
   if (!moduleData) {
      moduleData = await fs.readFile(`${path.join(sourceRoot, normalizedCurrentDependency)}.js`, 'utf8');
   }
