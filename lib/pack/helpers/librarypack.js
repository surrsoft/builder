 ...
 * Проверяем как компоненты, так и шаблоны, поскольку и то и то должно упаковываться
 * внутрь библиотеки.
...
function isInternalDependency(libDirectory, dependency) {
   return dependency.startsWith(libDirectory) || dependency.startsWith(`wml!${libDirectory}`);
}
