...
function gzip(data) {
   return new Promise((resolve, reject) => {
      ...
      /**
       * В качестве данных при выполнении функции в воркере передаётся
       * не буффер, как мы ожидаем, а обьект вида {type: 'Buffer', data: {Array}}.
       * Поэтому нам необходимо извлечь эти данные и превратить их обратно в буффер.
       * В случае передачи в качестве аргумента буффера или строки передаём на архивацию как есть.
       */
      let dataToZip;
      if (typeof data === 'object' && data.data) {
         dataToZip = Buffer.from(data.data);
      } else {
         dataToZip = data;
      }

      zlib.gzip(dataToZip, gzOptions, (err, compressed) => {
         if (err) {
            reject(err);
         } else {
            resolve(compressed);
