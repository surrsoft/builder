...
/**
 * Проверяет относится ли файл к локазилации.
 * @param moduleInfo - инфрормация о интерфейсном модуле
 * @param file - класс типа file
 * @returns {boolean}
 */
function isLocalizationFile(moduleInfo, file) {
   if (moduleInfo.contents.availableLanguage) {
      const avlLang = Object.keys(moduleInfo.contents.availableLanguage);
      return avlLang.includes(file.basename.replace(file.extname, ''));
   }
