...
/**
 * not all tensor servers have Node.Js 10.14.2. For some reasons at the right
 * moment they are using 8.11.x Node instead of 10.14.2 and cant update to the latest LTS-version
 * Dont require iltorb library in this case, requiring of this library in downgraded Node.js
 * version throws fatal error. Example
...
try {
   ...
   logger.warning(`Your Node.JS version(${process.version}) in outdated! Please reinstall it to 10.14.2 or newer for builder properly work!`);
}
...
function brotli(data) {
   return new Promise((resolve, reject) => {
      iltorb.compress(data, { quality: 7 }, (err, compressed) => {
         if (err) {
            reject(err);
         } else {
            resolve(compressed);
...
/**
 * returns new text content with replaced strings by given indexes.
 * if we have content to be replaced, just build result code with it using
 * simple string concatenation, without AST code generators. Reason - AST code
 * generator loses typescript comments, or in 1 working case duplicates them.
 * @param{String} text - current module content
 * @param{Array} contentToReplace - set of content with indexes to be replaced
 * @returns {string}
 */
function replaceStringsByIndex(text, contentToReplace) {
   let newCode = '';

   newCode += text.slice(0, contentToReplace[0].start);

   for (let i = 0; i < contentToReplace.length; i++) {
      newCode += contentToReplace[i].code;
      newCode += text.slice(
         contentToReplace[i].end,
         (contentToReplace[i + 1] && contentToReplace[i + 1].start) || ''
      );
   }
   newCode += text.slice(contentToReplace[contentToReplace.length - 1].end, text.length);
   return newCode;
}
