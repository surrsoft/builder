 ...
 * @param fullPath
 */
function removeFileFromVersionedMeta(versionedMeta, fullPath) {
   let removeFromVersioned;
   versionedMeta.forEach((versionedModule) => {
   ...
   if (removeFromVersioned) {
      ...
      const moduleIndex = versionedMeta.indexOf(removeFromVersioned);
      versionedMeta.splice(moduleIndex, 1);
   }
}
 ...
 * @returns {boolean}
 */
function needToRemoveModuleForDesktop(prettyPath, isRetailOrPrestoOffline) {
