...
/**
 * Get compressed in brotli for current input data.
 * Throws error if compress failed.
 * Compress quality setted to 7 - optimal by compressing time and result size.
 * @param{ArrayBuffer} data - current file buffer
 * @returns {Promise<any>}
 */
function brotli(data) {
   return new Promise((resolve, reject) => {
      compress(data, { quality: 7 }, (err, compressed) => {
         if (err) {
            reject(err);
         } else {
            resolve(compressed);
