...
(function () {
   ...
   module.exports = {
      createResultDict: function(g, task) {
         ...
         jsonOutput = path.join(dirName, '/../../../data/temp');

         if (grunt.file.isDir(jsonOutput)) {
         ...
         child_process.exec(dirName + '/../node_modules/sbis3-genie-server/bin/node.exe ' + dirName + '/json-generation-env.js', {
            env: {
               WS: ws,
               CLOUD: cloud,
               OUTPUT: jsonOutput
            ...
            if (paths.length == 1 && grunt.file.isFile(modules)) {
               try {
                  ...
                  grunt.log.error('Can\'t read ' + modules);
                  return;
               }
            }
            ...
            for (i = 0, len = paths.length; i < len; i++) {
               if (!paths[i]) continue;
               try {
                  findComponentsAndProperties(paths[i], jsonOutput);
               } catch(e) {
                  grunt.log.error(e);
               }
            }
            ...
            if (grunt.file.isDir(path.dirname(out))) {
               ...
               grunt.fail.warn('Could\'t create file ' + out);
            }
