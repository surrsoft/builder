...
function getRequireName(filePath) {
   ...
   while (pathParts.length !== 0 && !requireName) {
      ...
      requireName = requireNamesMap.get(filePathPart);
      pathParts.pop();
   }
...
function getPrettyPath(filePath) {
   ...
   if (requireNameToReplace) {
      resultPath = resultPath.replace(requireNameToReplace, requireNamesMap.get(requireNameToReplace));
   }
   return resultPath;
}
