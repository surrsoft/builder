    ...
    cfg.replace = {
        ...
        html: {
            ...
            replacements: [{
                ...
                from: /(<script[^>]+[\s][\w]+[\s]*=[\s]*)((?:"|')(?:[a-z]+(?!:\/)|\/|\.\/|%[^}]+})[\w\/+-.]+(?:\.\d+)?)(\.js)/ig,
                to: function (matchedWord, index, fullText, regexMatches) {
                    // ignore cdn and data-providers
                    if (regexMatches[1].indexOf('cdn/') > -1 || regexMatches[1].indexOf('http://') > -1
                        || regexMatches[1].indexOf('https://') > -1 || !/^src|^href/i.test(matchedWord)) {
                        return matchedWord;
                    }
                    return regexMatches[0] + regexMatches[1] + '.v' + grunt.option('versionize') + regexMatches[2];
                }
