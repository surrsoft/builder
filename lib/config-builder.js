module.exports = function (grunt, app) {
   app = app.indexOf('/') == 0 ? app.replace('/', '') : app; //удаляем ведущий слэш для старых тасок
   var appRoot = app && app.lastIndexOf('/') != app.length - 1 ? app + '/' : app != '/' ? app: '';
   ...
   cfg['xml-deprecated'] = {
      files: ['**/*.xml.deprecated']
   };

   cfg['html-deprecated'] = {
      files: ['**/*.html.deprecated']
   };
