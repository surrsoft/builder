...
/**
 * get variables import for old themes
 */
...
function getCurrentImports(filePath, theme, gulpModulesPaths) {
   ...
   /**
    * Object for theme can be declared without any path of it. F.e. default theme'll be resolved as 'online'
    * (it'll be resolved as online by default in case of usage old theme scheme), but interface module
    * 'SBIS3.CONTROLS'(source of theme online) don't exist in current project.
    */
   if (theme.path && filePath === `${helpers.unixifyPath(path.join(theme.path, theme.name))}.less`) {
      return imports;
   }

   const variablesImport = getThemeImport(theme);
   if (variablesImport) {
