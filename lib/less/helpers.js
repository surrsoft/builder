...
/**
 * Temp decision to provide backward compatibility with
 * old theme's scheme compiling
 * TODO remove it after task completion
 * https://online.sbis.ru/doc/a4a2c31b-5247-4afe-b3d7-f698dc2269bd
 * @type {string[]}
 */
const defaultThemes = ['Controls-theme', 'Controls-default-theme'];
 ...
 */
function getThemeImport(theme, defaultThemeObject) {
   const importLessName = builderConstants.oldThemes.includes(theme.name) ? '_variables' : theme.name;
   if (theme.isDefault && defaultThemes.includes(theme.variablesFromLessConfig)) {
      const defaultImport = helpers.unixifyPath(
         path.join(defaultThemeObject.module, defaultThemeObject.dirname, defaultThemeObject.name)
      );
   ...
   if (defaultThemeObject.module === 'Controls-theme') {
 ...
 * 2) if Controls-theme module is existing in current project, use mixins of it
 * 3) if nothing of above described but Controls-default-theme interface module is included, use mixins of it
 * 4) if none of above described is included, return empty string.
 * @param{Object} gulpModulesPaths - all interface modules of current project with physical paths for each one
 * @param{Object} defaultThemeObject - all meta for current theme
 * @param{Object} oldBuild - takes true if we are building less in old theme scheme
 * @returns {string}
 */
function getMixinImport(gulpModulesPaths, defaultThemeObject, oldBuild) {
   if (gulpModulesPaths.hasOwnProperty('SBIS3.CONTROLS') && oldBuild) {
      return '@import "SBIS3.CONTROLS/themes/_mixins";';
   }
   if (defaultThemeObject.module) {
      const mixinImport = helpers.unixifyPath(
         path.join(defaultThemeObject.module, defaultThemeObject.dirname, 'helpers/_mixins')
      );
...
function getCurrentImports(filePath, theme, gulpModulesPaths) {
   ...
   const controlsDefaultThemeIncluded = gulpModulesPaths.hasOwnProperty('Controls-default-theme');
   const defaultThemeObject = {};
   if (controlsThemeIncluded) {
      defaultThemeObject.module = 'Controls-theme';
      defaultThemeObject.dirname = 'themes/default';
      defaultThemeObject.name = 'default';
   } else if (controlsDefaultThemeIncluded) {
      defaultThemeObject.module = 'Controls-default-theme';
      defaultThemeObject.dirname = '';
      defaultThemeObject.name = '_theme';
   }
   const variablesImport = getThemeImport(theme, defaultThemeObject);
   if (variablesImport) {
      imports.push(variablesImport);
   }
   imports.push(getMixinImport(gulpModulesPaths, defaultThemeObject, theme.isDefault));
