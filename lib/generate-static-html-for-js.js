         ...
         fs.readFile(filePath, async(err, text) => {
            if (err) {
               ...
               try {
                  ...
                  error.message = `Ошибка при обработке файла ${filePath}: ${error.message.replace(dblSlashes, '/')}`;
                  reject(error);
               }
            }
         });
...
async function replaceIncludes(text, modules) {
   ...
   replaceMapIncludes.forEach((value, key) => {
      text = text.replace(key, cache[helpers.prettifyPath(value)]);
   });
   return text;
}
...
function replaceConstant(text, componentInfo, config, replacePath) {
   try {
      if (replacePath) {
         ...
         text = text.replace(APPLICATION_ROOT, config.urlServicePath);
         text = text.replace(SBIS_ROOT, config.urlServicePath + config.wsPath);
         text = text.replace(RESOURCE_ROOT, config.urlServicePath + 'resources/');
         text = text.replace(SERVICES_PATH, config.servicesPath || config.urlServicePath + 'service/');
...
function generateHtml(file, contents, componentInfo, config, modules, replacePath) {
...
function generateStaticHtmlForJs(file, componentInfo, contents, config, modules, replacePath) {
   return new Promise(function(resolve, reject) {
      if (
         ...
         generateHtml(file, contents, componentInfo, config, modules, replacePath).then(
            result => {
               resolve(result);
            },
            error => {
