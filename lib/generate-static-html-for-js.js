...
function replaceConstant(text, componentInfo, config, replacePath) {
   ...
   try {
      if (replacePath) {
         ...
         newText = newText.replace(SERVICES_PATH, config.servicesPath || `${config.urlServicePath}service/`);
...
async function generateStaticHtmlForJs(file, componentInfo, contents, config, modules, replacePath) {
