...
function generateHtml(contents, componentInfo, config, modules, forPresentationService) {
   return new Promise(async(resolve, reject) => {
      try {
         ...
         const result = await loadFileAndReplaceIncludes(templatePath, modules);
         resolve({
...
function generateStaticHtmlForJs(componentInfo, contents, config, modules, forPresentationService) {
   return new Promise(function(resolve, reject) {
      if (componentInfo.hasOwnProperty('webPage') && componentInfo.webPage.hasOwnProperty('outFileName') &&
         ...
         generateHtml(contents, componentInfo, config, modules, forPresentationService)
            .then(
               result => {
                  resolve(result);
               },
