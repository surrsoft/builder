...
function loadFileAndReplaceIncludes(filePath, resources) {
...
async function replaceIncludes(text, resources) {
   ...
   while (result) {
      ...
      promisesLoadFiles.push(loadFileAndReplaceIncludes(file, resources));
...
function generateHtml(contents, componentInfo, config, resourcesPath, forPresentationService) {
   return new Promise(async(resolve, reject) => {
      try {
         ...
         if (!htmlTemplate) {
            ...
            templatePath = path.join(resourcesPath, transliterate(htmlTemplate));
         }

         const result = await loadFileAndReplaceIncludes(templatePath, resourcesPath);
         resolve({
            outFileName: outFileName,
            text: replaceConstant(result, componentInfo, config, forPresentationService)
         });
      } catch (error) {
         reject(error);
      }
...
function generateStaticHtmlForJs(componentInfo, contents, config, resourcesPath, forPresentationService) {
   return new Promise(function(resolve, reject) {
      if (componentInfo.hasOwnProperty('webPage') && componentInfo.webPage.hasOwnProperty('outFileName') &&
         ...
         generateHtml(contents, componentInfo, config, resourcesPath, forPresentationService)
            .then(
               result => {
                  resolve(result);
               },
