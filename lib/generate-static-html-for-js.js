 ...
 * @param{Boolean} isRootTemplate - current dependency is the main template for current static html
 */
...
function findFileInModules(relativePath, modules, moduleInfo, isRootTemplate) {
   ...
   if (isRootTemplate && checkResult) {
      throw new Error(`External Interface module "${moduleName}" usage(old webpage html). Check for it existance in "${moduleInfo.name}" interface module dependencies`);
   }
...
async function generateStaticHtmlForJs(file, componentInfo, moduleInfo, config, modules, replacePath) {
   ...
   const templatePath = findFileInModules(htmlTemplate, modules, moduleInfo, true);
