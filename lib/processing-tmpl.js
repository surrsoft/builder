...
async function buildHtmlTmpl(text, fullPath, relativeFilePath, componentsProperties, splittedCore = true, replacePath, urlServicePath) {
   ...
   // %{SERVICES_PATH} будет заменен сервисом представлений. Если он не используется,
   // передаем путь самостоятельно (например для desktop-приложений)
   const servicesPath = replacePath ? `${urlServicePath}service/` : '%{SERVICES_PATH}';
