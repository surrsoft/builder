...
async function runCompilerAndCheckForErrors(sourceDirectory, redirectOutputCommand = '') {
   ...
   try {
      await exec(
         `${heapSizeCommand} && node ${sabyTypescriptDirectory}/cli.js --compiler --noEmit ${redirectOutputCommand}`,
         processOptions
      );
      logger.info('TypeScript compilation was completed successfully!');
      result = [];
   } catch (error) {
      // TODO redirect errors list from process.stdout into a single txt-file. stdout has a limit of messages and can loose a part of errors if there is a bunch of them
...
async function typescriptCompiler(taskParameters, output) {
   ...
   const tsErrors = await runCompilerAndCheckForErrors(sourceDirectory, redirectOutputCommand);
