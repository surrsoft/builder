...
async function runCompilerAndCheckForErrors(sourceDirectory) {
    ...
    * files with "saby-typescript" command for proper "tsc" command execute.
    */
   await fs.ensureSymlink(path.join(process.cwd(), 'node_modules/saby-typescript/configs/es5.json'), path.join(sourceDirectory, 'tsconfig.json'));
   await fs.ensureSymlink(path.join(process.cwd(), 'node_modules/saby-typescript/tslib.js'), path.join(sourceDirectory, 'tslib.js'));
   await fs.ensureSymlink(path.join(process.cwd(), 'node_modules/saby-typescript/tslint/index.json'), path.join(sourceDirectory, 'tslint.json'));
...
async function typescriptCompiler(taskParameters) {
   ...
   if (tsErrors.length > 0) {
      ...
      logger.warning('tsc command for current project was completed with errors. Check logs');
   }
}
