...
async function runCompilerAndCheckForErrors(sourceDirectory) {
    ...
    */
   const tsBinariesPath = path.join(require.resolve('saby-typescript'), 'node_modules', 'typescript', 'bin');
   process.env.PATH = `${tsBinariesPath}:${process.env.PATH}`;
...
async function typescriptCompiler(taskParameters) {
   ...
   if (tsErrors.length > 0) {
      ...
      if (taskParameters.config.isSbisPlugin) {
         ...
         logger.info(tsCompletedMessage);
      }
   }
}
