...
async function typescriptCompiler(taskParameters) {
   ...
   if (tsErrors.length > 0) {
      ...
      await fs.writeJson(
         path.join(logFolder, 'compilation-errors.json'),
         tsErrors
      );
      ...
      tsErrors.forEach((currentError) => {
         ...
         /**
          * Don't log errors in Sbis Plugin because of problem with tsc configuration
          * TODO remove it after task completion
          * https://online.sbis.ru/opendoc.html?guid=77afe3f3-e22e-46ce-8355-6f73c135f2e9
          */
         const level = !taskParameters.config.isSbisPlugin &&
            CRITICAL_ERRORS.some(message => currentError.startsWith(message)) ? 'error' : 'info';
      ...
      if (overallLevel === 'error') {
         logger[overallLevel]('TypeScript compilation was completed with errors. Check logs for details.');
      }
   }
}
