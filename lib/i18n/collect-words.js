...
function findWordsInWsExpert(words, text) {
   ...
   text.replace(wsExpertComments, function(str, commentContent) {
      commentContent.replace(translate, function(str, key) {
         posCtx = key.indexOf('@@');
         if (posCtx > -1) {
            getWords(words, key.substr(0, posCtx), key.substr(posCtx + 2));
         } else {
            getWords(words, key, '');
         }
         return str;
...
async function findWordsInTMPL(words, text, filePath) {
   let result = await tmplLocalizator.parseTmpl(text, filePath, globalComponentsProperties);
   if (result.hasOwnProperty('words')) {
      result = result.words || [];
   }
   Array.prototype.push.apply(words, result);
}
...
async function collectWords(modulePath, filePath, text, componentsProperties) {
   ...
   if (ext === '.js') {
      ...
      await findWordsInTMPL(words, text, filePath);
   }
