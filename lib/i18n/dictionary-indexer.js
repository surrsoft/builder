...
const cssHelpers = require('grunt-wsmod-packer/lib/cssHelpers.js');
...
class DictionaryIndexer {
   constructor(localizations) {
      ...
      //список всех css локализации. {<имя модуля.локаль.css>: [{filePath:<путь до файла>, text: <текст файла>}, ...], ...}
      this.cssStore = {};
   }

   addLocalizationJson(modulePath, filePath, locale) {
      if (this.localizations.includes(locale)) {
         const expectedFilePath = path.join('lang', locale, locale + '.json');
   ...
   extractMergedCSSCode(modulePath, locale) {
      ...
      return bumpImportsUp(this.cssStore[key].map(function(cssObj) {
         return resolveUrl(path.dirname(modulePath), cssObj.filePath, cssObj.text);
      }).join('\n'));
   }
