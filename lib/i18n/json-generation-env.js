...
//запускаем не больше 4 воркеров. иначе падают сервера сборки на переполнении памяти.
//на сервере сборки 12 ядер и может одновременно запускаться 3 и более сборки с builder'ом.
const maxWorkerCount = Math.min(Math.max(2, os.cpus().length), 4),
