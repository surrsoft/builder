...
(function() {
   ...
   function readDict(resourceRoot, nameDir, lang) {
      ...
      if (fs.existsSync(dictPath)) {
         try {
            ...
            grunt.log.error(`Ошибка чтения словаря по пути ${dictPath}.`);
            grunt.log.ok(e);
            return '';
         }
      } else {
   ...
   module.exports = {
       ...
       * @param grunt
       * @param data
       * @param avalibLang - доступные языки локализации
       */
      normalize: function(grunt, data, availabLang) {
         let
            langs = getAvailableLang(availabLang),
