...
async function normalizeDicts(dictPaths) {
   const originalDicts = {};
   await pMap(dictPaths, async(dictPath) => {
      let result = {};
   ...
   const globalDict = Object.assign({}, ...dictPaths.map(dictPath => {
      return originalDicts[dictPath];
   }));
   ...
   await pMap([...needWrite], async(dictPath) => {
      try {
...
async function normalize(resourceRoot, languages) {
   ...
   languages.forEach(function(lang) {
      const dictPaths = namesDir.map(nameDir => {
         return path.normalize(path.join(resourceRoot, nameDir, 'lang', lang, lang + '.json'));
      });
