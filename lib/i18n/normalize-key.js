...
function readDict(resourceRoot, nameDir, lang) {
   const dictPath = path.normalize(path.join(resourceRoot, nameDir, 'lang', lang, lang + '.json'));
...
function writeDict(resourceRoot, nameDir, data, lang) {
   ...
   try {
      ...
      logger.debug(`Не удалось записать в файл: ${dictPath}. Stack: ${error.stack.toString()}`);
   }
}
 ...
 */
function normalize(data, availableLang) {
   let
   ...
   langs.forEach(function(lang) {
      ...
      namesDir.forEach(function(dir) {
         const dict = readDict(resourceRoot, dir, lang);
