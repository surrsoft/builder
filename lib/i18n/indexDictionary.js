                ...
                firstLevelDirs.forEach(function (fstLvlD) {
                    ...
                    for (var langKey in levelLocalDict) {
                        ...
                        if (levelLocalDict[langKey].levelDicts.length || levelLocalDict[langKey].levelCss.length || levelLocalDict[langKey].levelCountryCss.length) {
                            ...
                            var
                               relativeDictPath = path.relative(resourceRoot, dictPath).replace(dblSlashes, '/'),
                               relativeDictPathDep = '/' + path.relative(applicationRoot, dictPath).replace(dblSlashes, '/'),
                               relativeCssPath,
                               relativeCounryPath,
                            ...
                            if (Object.keys(mergedDicts).length) {
                                dictModuleDeps.push('"text!' + relativeDictPathDep + '"');
                                dictModuleArgs.push('dict');
                                dictModuleContent += 'i18n.setDict(JSON.parse(dict), "text!' + relativeDictPathDep + '", "' + langKey + '");';
