...
(function () {
    ...
    module.exports = {
        ...
        indexDict: function (grunt, languages, data, done) {
                    ...
                    for (var langKey in levelLocalDict) {
                        ...
                        if (levelLocalDict[langKey].levelDicts.length || levelLocalDict[langKey].levelCss.length || levelLocalDict[langKey].levelCountryCss.length) {
                            ...
                            if (mergedCss) {
                                ...
                                dictModuleDeps.push('"native-css!' + relativeCssPath + '"');
                                dictModuleArgs.push('css');
                                dictModuleContent += 'if(i18n.getLang()=="' + langKey + '"){requirejs(["native-css!' + relativeCssPath + '"]);}';
                            ...
                            if (mergedCountryCss) {
                                ...
                                dictModuleDeps.push('"native-css!' + relativeCounryPath + '"');
                                dictModuleArgs.push('country');
                                dictModuleContent += 'if(i18n.getLang().indexOf("' + langKey + '")>-1){requirejs(["native-css!' + relativeCounryPath + '"]);}';
                            ...
                            if(dictModuleDeps.length) {
                                ...
                                fs.writeFileSync(dictPath.replace('.json', '.js'), 'define("' + dictMouduleName + '", [' + dictModuleDeps.join() + '], ' +
                                   'function(' + dictModuleArgs.join() + ') {' + dictModuleContent + '});requirejs(["Core/core-init-min"], function() {requirejs(["' + dictMouduleName + '"])});');
                            }
            ...
            $ws._const.availableLanguage = availableLang;
            $ws._const.defaultLanguage = defLang;
            $ws._const.dictionary = jsDict;
