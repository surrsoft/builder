...
(function () {
    ...
    */
     ...
     * @returns {boolean}
     */
    function checkExistMergeDict(mergeDictPath, allDictPath, moduleName) {

        let mergeDictPathes = mergeDictPath.replace(dblSlashes,'/'),
        flag = false;

       allDictPath.forEach(function (pathDict) {
                if (~pathDict.indexOf(mergeDictPathes)) {
                   if (allDictPath.length > 1) {
                        grunt.log.warn(`In the directory ${moduleName} exists the merged dictionary. Remove all the not needed dictionaries in the directory.`);
                    }
                    return flag = true;
                }
        });
        ...
        if (allDictPath.length > 0) {
            grunt.log.warn(`In the directory ${moduleName} not exists the merged dictionary. Merge all dictionaries into one dictionary and add it to ${mergeDictPath}.`);
            return false;
        } else {
            return true;
        }
