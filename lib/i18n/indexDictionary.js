...
(function () {
     ...
     * @returns {Object}
     */
    function getDictionaryPath(grunt, levelLocalDict, lang, _path, applicationRoot) {
        ...
        if (dir) {
            ...
            files.forEach(function (modPath) {
                ...
                if (moduleRegExp.test(modPath)) {
                    ...
                    if (text) {
                        getModuleName(text.toString(), function (modName) {
                            if (modName) {
    ...
    function mergeDicts(grunt, packageRoot, levelDicts) {
        var allWords = {};
    ...
    function mergeCss(grunt, packageRoot, levelLocalDict) {
        var levelCss = levelLocalDict.levelCss,
            levelCountryCss = levelLocalDict.levelCountryCss,
            css, country;

        css = bumpImportsUp(levelCss.map(function(cssPath){
            return resolveUrl(packageRoot, path.dirname(cssPath), grunt.file.read(cssPath));
        }).join('\n'));

        country = bumpImportsUp(levelCountryCss.map(function(cssPath){
            return resolveUrl(packageRoot, path.dirname(cssPath), grunt.file.read(cssPath));
        }).join('\n'));
                    ...
                    grunt.file.recurse(fstLvlD, function (absPath) {
                        ...
                        if (language && language[1]) {
                            ...
                            if (lang in availableLang) {
                                ...
                                getDictionaryPath(grunt, levelLocalDict[lang], lang, absPath, data.cwd);
                            }
                        }
                    });
                    ...
                    for (var langKey in levelLocalDict) {
                        ...
                        if (levelLocalDict[langKey].levelDicts.length) {
                            var country = langKey.substr(3, 2),
                                ...
                                mergedDicts = mergeDicts(grunt, dictPath, levelLocalDict[langKey].levelDicts),
                                mergedCss = mergeCss(grunt, cssPath, levelLocalDict[langKey]).css,
                                mergedCountryCss = mergeCss(grunt, countryCssPath, levelLocalDict[langKey]).country;
