...
(function () {
    ...
    function readDict (grunt, dictPath) {
        try {
            return JSON.parse(fs.readFileSync(dictPath));
        } catch (err) {
            if ('ENOENT' === err.code) {
                grunt.log.warn('i18n: ' + err.message);
                return {};
            }
            grunt.fail.fatal('i18n: ' + err.message);
        }
    }
            ...
            if (Object.keys(availableLang).length) {
                firstLevelDirs.forEach(function (fstLvlD) {
                    ...
                    for (var langKey in levelLocalDict) {
                        ...
                        if (levelLocalDict[langKey].levelCss.length || levelLocalDict[langKey].levelCountryCss.length) {
                            var country = langKey.substr(3, 2),
                                ...
                                mergedDicts = readDict(grunt, dictPath),
