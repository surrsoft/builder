...
(function () {
    ...
    * @returns {Object}
    */
   function getDictionaryPath(grunt, obj, lang, _path, subDir) {
      ...
      if (dir) {
         ...
         grunt.file.recurse(dir, function(modPath) {
            if (moduleRegExp.test(modPath)) {
               ...
               if (text) {
                  getModuleName(text, function(modName) {
                     var moduleName = content && content.jsModules && content.jsModules[modName] || '';
                     if (modPath && modPath.replace(/\\/g,'/').indexOf(moduleName.replace(/\\/g,'/'))) {
                        if (modName) {
                           obj[modName + '.' + lang + '.json'] = _path;
            ...
            grunt.file.recurse(path.join('./', data.application), function(absPath) {
               ...
               if (language && language[1]) {
                  ...
                  if (lang in availableLang) {
                     getDictionaryPath(grunt, jsDict, lang, absPath, data.application);
                  }
               }
            });
         }
