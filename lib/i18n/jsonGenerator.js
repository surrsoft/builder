...
function generateJsDoc(modules, jsonOutput, cb) {
   ...
   const jsDocWorker = childProcess.fork(
      path.join(__dirname, '..', 'jsDoc/json-generation-env.js'),
      [
         'input=' + modules,
         'cache=' + jsonOutput
      ]);
...
function readAllJSON(directory) {
   ...
   function readPropertiesJSON(fileObj) {
      ...
      if (fs.existsSync(modPath)) {
         try {
            return fs.readFileSync(JSON.parse(modPath));
         } catch (e) {
            logger.error('Can\'t read ' + modPath);
         }
      }
      return {};
   }
...
module.exports = {
   jsonGenerator: function(modules, jsonOutput, done) {
