...
const
   path = require('path'),
   parser = global.requirejs('Core/markup/ParserUtilities'),
   runJsonGenerator = require('./run-json-generator'),
...
function translateDOM(text) {
   let dom;
   try {
      logger.debug(`prepareXHTML::translateDOM::currentPath -> ${currentPath}`);
      dom = parser.parse(text);
      enumChildNodes(dom);
   } catch (e) {
      logger.error({
         message: 'Can\'t parse DOM in file',
 ...
 * @param done
 */
function prepareXHTML(g, data, done) {
   let text;
   ...
   runJsonGenerator.run(modules, jsonOutput, function(error) {
      if (error) {
         grunt.log.error(error);
      }

      componentsProperties = runJsonGenerator.componentsProperties();
