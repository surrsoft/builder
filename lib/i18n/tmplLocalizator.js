...
var tmplParser = global.requirejs ? global.requirejs('Core/tmpl/tmplstr') : undefined;
...
function warning (err, filePath) {
   // gutil.log(gutil.colors.black.bgYellow(`gulp-sbis-tmpl-build: Tmpl Localization/`), `${err.message}, in file: ${filePath}`);
}
...
function parseTmpl(_grunt, tmplMarkup, currentPath) {
    grunt = _grunt;
    if (!Deferred) Deferred = global.requirejs('Core/Deferred');
    ...
    try {
       componentsProperties = readAllJSON(jsonOutput);
        ...
        if (_grunt) { // заглушка для gulp
            ...
            warning(e, currentPath);
        }
    }
    if (!tmplParser) tmplParser = global.requirejs('Core/tmpl/tmplstr');
    if (!configModule) configModule = global.requirejs('Core/tmpl/config');
