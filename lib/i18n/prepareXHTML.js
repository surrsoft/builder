...
(function () {
    ...
    function readPropertiesJSON(modName) {
        ...
        if (grunt.file.exists(modPath)) {
            ...
            grunt.log.error('Module properties doesn\'t exists ' + modPath);
        }
    ...
    function translateDOM(text) {
        var dom;
        try {
            dom = parser.parse(text, currentPath);
            enumChildNodes(dom);
        } catch (e) {
    ...
    function translateComponent(node) {
        ...
        if (transProp.length) {
            ...
            try {
                ...
                grunt.fail.fatal('Error parsing component configuration: ' + e);
            }
        }
