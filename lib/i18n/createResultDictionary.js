...
(function () {
   ...
   function pushKey(key, ctx, module) {
      ...
      if (!temp || isDoT2.test(temp) || isComment.test(temp) || isDirective.test(temp) || isDoctype.test(temp)) {
         return;
      }

      if (translate.test(temp)) {
         // Надо удалить {[ ]}
         temp = (matches = temp.match(translate)) ? matches[1] : temp;
      }
   ...
   function propertiesParser(transPropXPath, object, propPath, moduleName) {
      ...
      Object.keys(object).forEach(function(key) {
         ...
         if (elem.options) {
            ...
            var properties = componentsProperties[elem.type];
            if (!properties && elem.type) {
               var
                  modName = elem.type,
   ...
   module.exports = {
      createResultDict: function(g, task) {
         ...
         ws = path.join(__dirname, '../../../../ws/');
         jsonOutput = path.join(__dirname, '../../../../data/temp-' + uuid.v4());

         genJsDoc(grunt, modules, ws, jsonOutput, function(error) {
            if (error) {
