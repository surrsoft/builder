...
(function () {
   ...
   function checkCache(pathToSource) {
      var mtime = new Date(fs.statSync(pathToSource).mtime),
         fromCache;

      if (pathToSource in i18nCacheContent && (+mtime === +new Date(i18nCacheContent[pathToSource].mtime))) {
         i18nCacheContent[pathToSource].words.forEach(function (word) {
   ...
   module.exports = {
      createResultDict: function(g, done) {
         ...
         i18nCacheContent = grunt.file.exists(i18nCache) ? fs.readFileSync(i18nCache, 'utf-8') || '{}' : '{}';
