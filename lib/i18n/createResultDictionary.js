...
var xhtmlParser = global.requirejs('Core/markup/ParserUtilities');
var Deferred = global.requirejs('Core/Deferred');
var ParallelDeferred = global.requirejs('Core/ParallelDeferred');
var genJsDoc = require('./../jsDoc/generateJsDoc');
var fs = require('fs');


var tmplParser = global.requirejs('Core/tmpl/tmplstr');
var configModule = global.requirejs('Core/tmpl/config');
var resolverControls = function resolverControls(path) {
    return 'tmpl!' + path;
};
function parseTmpl(tmplMarkup, currentPath, componentsProperties) {
    ...
    }).handle(function (traversed) {
        def.callback(traversed.words);
    }, function (error) {
        console.error(error);
        def.errback(error);
    });
...
(function () {
    ...
    function findWordsInXHTML(cwd) {
        var sourceFiles = grunt.file.expand({cwd: cwd}, ['**/*.xhtml', '**/*.tmpl']),
            absPath, text;
    ...
    function findWordsInDOM(text, currentPath) {
        ...
        try {
            ...
            if (/\.tmpl$/.test(currentPath)) {
                readAllJSON();
                words.push(parseTmpl(text, currentPath, componentsProperties));
    ...
    function readAllJSON() {
        ...
        if (!jsonLoaded) {
            fs.readdirSync(jsonOutput).forEach(function (file) {
                file = file.replace(/\.json$/g, '');
                readPropertiesJSON(file);
                if (componentsProperties[file] &&
                    componentsProperties[file].properties &&
                    componentsProperties[file].properties["ws-config"] &&
                    componentsProperties[file].properties["ws-config"].options) {

                } else {
                    delete componentsProperties[file];
                }
            });
