...
describe('packer loaders', () => {
   ...
   describe('i18n loader', () => {
      it('without dependencies', async() => {
         const i18nRoot = path.join(loadersRoot, 'i18nLoader');
         const module = {
            fullName: 'Module_localization',
            plugin: 'i18n',
            availableDict: {
               'en-US': true
            }
         };
         const result = await packLoaders.i18n(module, null, null, {});
         const correctResult = await fs.readFile(path.join(i18nRoot, 'defaultLocalization.js'), 'utf8');
         result.should.equal(correctResult);
      });
      it('without dictionaries', async() => {
         const i18nRoot = path.join(loadersRoot, 'i18nLoader');
         const module = {
            fullName: 'Module_localization',
            plugin: 'i18n'
         ...
         const result = await packLoaders.i18n(module, null, null, {});
         const correctResult = await fs.readFile(path.join(i18nRoot, 'localizationWithoutDictionaries.js'), 'utf8');
         result.should.equal(correctResult);
      });
      it('without moduleName', async() => {
         const i18nRoot = path.join(loadersRoot, 'i18nLoader');
         const module = {
            fullName: '',
            plugin: 'i18n',
         ...
         const result = await packLoaders.i18n(module, null, null, {});
         const correctResult = await fs.readFile(path.join(i18nRoot, 'localizationWithoutModuleName.js'), 'utf8');
         result.should.equal(correctResult);
      });

      it('with common dependencies', async() => {
         const i18nRoot = path.join(loadersRoot, 'i18nLoader');
         const module = {
            fullName: 'Module_localization',
            plugin: 'i18n',
         ...
         const result = await packLoaders.i18n(
            module,
            null,
            null,
            {
               availableLanguage: ['en-US', 'ru-RU'],
               defaultLanguage: 'ru-RU'
            }
         );
         const correctResult = await fs.readFile(path.join(i18nRoot, 'localizationWithDeps.js'), 'utf8');
         result.should.equal(correctResult);
      });
      it('with lang dependencies', async() => {
         const i18nRoot = path.join(loadersRoot, 'i18nLoader');
         const module = {
            fullName: 'Module_localization',
            plugin: 'i18n',
         ...
         const correctResult = await fs.readFile(path.join(i18nRoot, 'localizationWithLangDeps.js'), 'utf8');
         result.should.equal(correctResult);
      });
   });
});
