...
var fs = require('fs');
var DepGraph = require('./../../lib/dependencyGraph');
...
var i18n = global.requirejs('Core/i18n');
var availableLangs = Object.keys(i18n.getAvailableLang());

var supportedPlugins = ['js', 'html', 'css', 'json', 'xml', 'text', 'native-css', 'browser', 'optional', 'i18n', 'tmpl'];
var pluginsOnlyDeps = ['cdn', 'preload', 'remote'];
var systemModules = ['module', 'require', 'exports'];
...
function needPlugin(meta) {
    if (meta.plugin == 'js') {
        ...
        var firstFolder = meta.module && meta.module.split('/')[0];
...
function getDependencies(grunt, fullPath, deps) {
    ...
    deps.forEach(function collectPlugins(i) {
        ...
        if (i.plugin == 'is') {
            var yesPlugin = i.moduleYes && i.moduleYes.plugin;
            var noPlugin = i.moduleNo && i.moduleNo.plugin;
            ...
            var inPlugin = i.moduleIn && i.moduleIn.plugin;
    ...
    try {
        if (dep.module) {
            if (dep.plugin == 'text' || dep.plugin == 'native-css') {
                ...
                var jsPath, cssPath, countryPath;
        ...
        if (pathToModule) {
            ...
            var ext = path.extname(pathToModule).substr(1);
...
function getComponentForTmpl(grunt, graph, applicationRoot, mod) {
    if (mod && mod.plugin === 'tmpl' && mod.fullPath) {
        try {
            ...
            var text = fs.readFileSync(mod.fullPath);
            if (text) {
                var tmplstr = global.requirejs('Core/tmpl/tmplstr');
                var arr = tmplstr.getComponents(text).map(function (dp) {
                    return {
                        type: 'Literal',
                        value: dp
                    };
                });
                var d = getDependencies(grunt, mod.fullPath, arr);
                var errMes = '-------- Parent name: "' + mod.fullName + '"; Parent path: "' + mod.fullPath + '"';
...
function getDependenciesForI18N(grunt, graph, applicationRoot, mod) {
    if (mod && mod.plugin === 'i18n' && mod.fullPath) {
        try {
            var deps = [];
            var resourceRoot = path.join(applicationRoot, 'resources');
            availableLangs.forEach(function(lang) {
            ...
            graph.addDependencyFor(mod.fullName, deps.map(function(dep) {return dep.fullName}));
        } catch(err) {
            grunt.log.warn(err);
        }
    }
                ...
                traverse(ast, {
                    enter: function getModuleNameAndDependencies(node) {
                        if (node.type == 'CallExpression' && node.callee.type == 'Identifier' &&
                            ...
                            var needToRegister = (meta.plugin == 'js' && (!jsModules[meta.module] || jsModules[meta.module] == fullPath)) || meta.plugin != 'js',
