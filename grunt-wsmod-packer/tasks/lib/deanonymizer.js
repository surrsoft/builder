...
function deanonymize(grunt, foundNames, taskDone) {
   const jsFiles = Object.keys(foundNames);
               ...
               traverse(ast, {
                  enter: function getModuleName(node) {
                     if (node.type == 'CallExpression' && node.callee.type == 'Identifier' &&
                        ...
                        const modulePath = foundNames[fullPath].replace(/\\/g, '/');
...
function findNames(grunt, anonymous, badRequireDeps, root, taskDone) {
   ...
   const foundNames = {};
   const anonKeys = Object.keys(anonymous);
   const keys = Object.keys(badRequireDeps);
...
function fillBadRequireDeps(fullPath, badRequireDeps, deps) {
   ...
   deps.forEach(function(dep) {
      const appRoot = process.env.ROOT;
