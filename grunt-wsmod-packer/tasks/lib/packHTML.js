...
var packInOrder = require('./packInOrder');
var domHelpers = require('./../../lib/domHelpers');
var indexer = require('./../../lib/index-project');

var ERROR_INDEX_FAILED = 100;
var ERROR_PACKING_FAILED = 101;
    ...
    async.eachLimit(htmlFileset, 1, function (htmlFile, done) {
        try {
            ...
            if (jsTarget || cssTarget) {
                var startNodes = getStartNodes(grunt, divs, application);
                ...
                packInOrder(dg, startNodes, root, path.join(root, application), false, {}, function (err, filesToPack) {
                    if (err) {
                        ...
                        var attr2ext = {'cssForLocale': 'css', 'dict': 'js'},
                            packages = {'css': [], 'js': []};
                        ...
                        Object.keys(filesToPack).forEach(function (key) {
                            if (filesToPack[key] !== null && typeof filesToPack[key] === 'object') {
                                ...
                                let generatedScript = generatePackage(key, grunt, filesToPack[key], getKey(buildNumber, key), packageHome, applicationRoot, root);
