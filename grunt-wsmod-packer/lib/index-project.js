...
function _findPath(from, to, path, storage, graph) {
    path.push(from);
    var deps = graph.getDependenciesFor(from);
    if (!deps.length) {
        path.pop();
    } else {
...
function _addDependency(store, dependency) {
    var ClassMapper = global.requirejs('Deprecated/ClassMapper');
...
function resolveOptions(ctor) {
    if (ctor) {
        var cMerge = global.requirejs('Core/core-merge');
        return cMerge(
                            ...
                            if (configAttr) {
                                (function (div, configAttr) {
                                    ...
                                    pdStep.push(resolveType(typename).addCallback(function (classCtor) {
                                        ...
                                        var cMerge = global.requirejs('Core/core-merge');
                                        var finalConfig = cMerge(baseConfig, config[0]);
                                        ...
                                        var failedModule = e.message && e.message.match(grabFailedModule);
                                        if (failedModule && failedModule[1]) {
                                            var failedPaths = findPath(typename, failedModule[1], graph);
