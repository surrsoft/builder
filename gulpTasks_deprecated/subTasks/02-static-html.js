...
function parseOpts(opts, application, replaceOpts, file) {
   ...
   if (!htmlTemplate) {
      gutil.log(`Using default template for output file ${outFileName}.html`);
   }
...
function parseObjectExpression(properties) {
   ...
   for (let i = 0, l = properties.length; i < l; i++) {
      obj[properties[i].key.name] = properties[i].value.value;
   }
   return obj;
}
      ...
      text = text.replace(INCLUDE, function(m, include) {
         ...
         for (let i = 0, l = modules.length; i < l; i++) {
            if (acc.hasPath(path.join(modules[i], '../', include))) {
               ...
               if (!acc.containsPath(filePath)) {
                  acc.loadFile(filePath);
               }
