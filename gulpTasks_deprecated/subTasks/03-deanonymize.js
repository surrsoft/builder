   ...
   function resolveName(dep) {
      ...
      if (rjsPaths[module]) {
         if (/^ws[\/\\]/.test(rjsPaths[module])) {
            ...
            for (let i = 0, l = opts.acc.modules.length; i < l; i++) {
               ...
               if (modulePathTranslited.endsWith(module)) {
                  resolvePath = path.join(modulePathTranslited, pathToFile);
               }
            }
         }
      ...
      if (resolvePath.startsWith('resources')) {
         ...
         for (let i = 0, l = opts.acc.modules.length; i < l; i++) {
            ...
            if (resolvePath.startsWith(mBaseTr)) {
               resolvePath = path.join(opts.acc.modules[i], '../', resolvePath);
            }
         }
      } else {
      ...
      for (let aPath in opts.acc.deanonymizeData.anonymous) {
         if (translit(aPath) == resolvePath) {
            foundNames[aPath] = dep;
         }
      }
   }
...
function deanonymize(foundNames, opts) {
   ...
   for (let file in acc) {
      if (acc[file] && acc[file].__anonymous) {
         ...
         if (wasModified) {
            opts.acc.setFileContents(file, codegen.generate(ast));
         }
      }
   }
}
