...
exports.traverse = (opts) => {
   ...
   if (node.type == 'CallExpression' && node.callee.type == 'Identifier' && node.callee.name == 'define') {
      ...
      if (modules[meta.module]) {
         modules[meta.module] = modules[meta.module].replace(/resources[\/\\]resources/, 'resources');
      }
      let needToRegister = (meta.plugin == 'js' && (!modules[meta.module] || modules[meta.module] == opts.file.dest)) || meta.plugin != 'js';

      if (!Array.isArray(modulesDeps[node.arguments[0].value].deps)) {
         modulesDeps[node.arguments[0].value].deps = [];
      }
...
function getDependenciesForI18N(opts, mod) {
   if (mod && mod.plugin === 'i18n' && mod.fullPath) {
      try {
         ...
         opts.acc.graph.addDependencyFor(mod.fullName, deps.map(function(dep) {
            return dep.fullName;
         }));
      } catch (err) {
         gutil.log(err);
