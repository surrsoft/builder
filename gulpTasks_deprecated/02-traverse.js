...
module.exports = opts => {
   ...
   return through2.obj(
      function(file, enc, cb) {
         if (file.isNull() || !('.js' === path.extname(file.relative))) {
            return cb(null, file);
         }
         if (file.isStream()) {
            return cb(new PluginError('gulp-sbis-traverse', 'Streaming not supported'));
         }
         if (!opts.acc) {
            return cb(new PluginError('gulp-sbis-traverse', 'acc option is required'));
         }
         if (file.path.endsWith('core-edo' + path.sep + 'builder.js')) {
            return cb(null, file);
         }
         let ast;
         try {
            ...
            enter: function(node) {
               if (node.type == 'CallExpression' && node.callee.type == 'Identifier' && node.callee.name == 'define') {
                  if (node.arguments[0].type == 'Literal' && typeof node.arguments[0].value == 'string') {
                     if (isModuleJs) {
                        contentsIsModuleJs({ acc: opts.acc, node: node, contents: opts.acc.contents, file: { base: file.base, relative: file.relative, __WS: file.__WS } });
                     }
                  }
               }
      ...
      function(cb) {
         ...
         if (needCollectDependencies) {
            collectDependencies.execute({ acc: opts.acc });
         }
