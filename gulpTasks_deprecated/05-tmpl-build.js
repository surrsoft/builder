...
module.exports = opts => {
   ...
   return through2.obj(
      function(file, enc, cb) {
         if (!tmplLocalizator) {
            tmplLocalizator = require('./../lib/i18n/tmplLocalizator');
         }

         if (file.isStream()) {
            return cb(new PluginError('gulp-sbis-tmpl-build', 'Streaming not supported'));
         }
         if (!opts.acc) {
            return cb(new PluginError('gulp-sbis-tmpl-build', 'acc option is required'));
         }
         if (!['.tmpl', '.html', '.xhtml'].some(ext => ext == path.extname(file.path))) {
            return cb(null, file);
         }
         if (file.sourceMap) {
            opts.sourcemap = true;
...
function execute(opts, ctx, cb) {
   ...
   let nodesRevert = {};
   for (let k in nodes) {
      if (k.startsWith('tmpl!') || k.startsWith('html!')) {
         nodesRevert[nodes[k].path] = k;
      }
   }
