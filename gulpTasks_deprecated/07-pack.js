...
module.exports = opts => {
   ...
   return through2.obj(
      function(file, enc, cb) {
         if (file.isStream()) {
            return cb(new PluginError('gulp-sbis-packwsmod', 'Streaming not supported'));
         }
         if (!opts.acc) {
            return cb(new PluginError('gulp-sbis-packwsmod', 'acc option is required'));
         }

         if (file.__MANIFEST__) {
            return cb(null, file);
         }
         ...
         if (file.__STATIC__) {
            if (!file.dest in opts.acc.packwsmod) {
               opts.acc.packwsmod[file.dest] = {};
            }
            if (!file.dest in opts.acc.packjscss) {
               opts.acc.packjscss[file.dest] = {};
            }
      ...
      function(cb) {
         ...
         if (!__packwsmod__.length && !__STATIC__.length) {
            return cb();
         }
         ...
         for (let i = 0, l = __packjscss__.length; i < l; i++) {
            ...
            for (let staticPath in opts.acc.packjscss) {
               if (need2bundle[staticPath]) {
                  continue mainLoop1;
               }
               if (!Array.isArray(opts.acc.packjscss[staticPath][ext])) {
                  opts.acc.packjscss[staticPath][ext] = [];
               }
         ...
         Promise.all(promises)
            .then(result => {
               ...
               if (!opts.acc.packwsmodXML) {
                  opts.acc.packwsmodXML = {};
               }
                  ...
                  staticFiles.forEach(sFile => {
                     ...
                     function packerJS(htmlDest, files) {
                        return [files.map(function(js) {
                           ...
                           if (!opts.acc.packjscss[htmlDest]) {
                              opts.acc.packjscss[htmlDest] = {};
                           }
                           if (!opts.acc.packjscss[htmlDest][ext]) {
                              opts.acc.packjscss[htmlDest][ext] = [];
                           }
