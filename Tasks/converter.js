      ...
      paths.forEach(function (input) {
         ...
         grunt.file.recurse(input, function (abspath) {
            ...
            if (!symlink || (i18n && (ext == '.xhtml' || ext == '.html'))) {
               try {
                  grunt.file.copy(abspath, path.join(resourcesPath, moduleName, transliterate(path.relative(input, abspath))));
               } catch (err) {
                  grunt.log.error(err);
               }
            } else {
               mkSymlink(abspath, path.join(resourcesPath, moduleName, transliterate(path.relative(input, abspath))));
            }
         });
      });
